...
for i,I in enumerate(ids):
	...
	#print(feat_onehot.shape, bp_prob[:,None].shape)
...
with tf.compat.v1.Session(config=config) as sess:
    saver = tf.compat.v1.train.import_meta_graph(str(args.meta_file_path) + 'models/tensorflow_model_single.meta')
    saver.restore(sess,str(args.meta_file_path) + 'models/tensorflow_model_single')
